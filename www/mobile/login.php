...
function do_recover() {
	...
	if(!empty($_POST['recover'])) {
		if (!ts_is_human()) {
			recover_error(_('o código de seguridade non é correcto'));
		} else {
			$error = false;
			$user=new User();
			if (preg_match('/.+@.+/', $_POST['email'])) {
				...
				recover_error(_('o email non é válido'));
				$error = true;
			}

			if(!$error && !$user->read()) {
				recover_error(_('o email non está relaccionado con ningunha conta'));
				$error = true;
			}
			if(!$error && $user->disabled()) {
				recover_error(_('conta deshabilitada'));
				$error = true;
			}
			if (!$error) {
	...
	if (!$sent) {
		...
		echo '<label for="name">'._('indica o email da cuenta').':</label><br />'."\n";
		echo '<input type="text" name="email" size="25" id="name" value="'.htmlspecialchars($_POST['email']).'" />'."\n";
		echo '<p>'._('(recibirás un e-mail que che permitirá editar os teus datos)').'</p>&nbsp;<br/>';
